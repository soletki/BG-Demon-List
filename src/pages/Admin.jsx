...
export default function AdminPage() {
	...
	async function fetchData() {
		try {
			...
			setRecords([
				{
					id: "9FG4fzKxv5Sv4LBzuhP3",
					levelName: "Acheron",
					playerName: "zInslanex",
					progress: 100,
					video: "https://www.youtube.com/watch?v=gBcDsB6wams",
					status: "pending"
				},
				{
					id: "2",
	...
	const filteredRecords = records.filter(record => {
		const playerMatch = record.playerName.toLowerCase().includes(playerFilter.toLowerCase());
		const levelMatch = record.levelName.toLowerCase().includes(levelFilter.toLowerCase());
		...
		<div className="admin-container">
			<div className="admin-main-container">
				...
				<div className="tab-nav">
					<button
						className={`tab-button ${activeTab === 'records' ? 'active-tab' : ''}`}
						onClick={() => setActiveTab('records')}
					>
						Records
					</button>
					<button
						className={`tab-button ${activeTab === 'claims' ? 'active-tab' : ''}`}
							...
							<div className="records-list">
								...
								<div className="record-items">
									{filteredRecords.map(record => (
										<div
											key={record.id}
											className={`record-item ${selectedRecord?.id === record.id ? 'selected-record' : ''}`}
											onClick={() => setSelectedRecord(record)}
										>
											<div className="record-info">
												<div className="record-title">{record.levelName}</div>
												<div className="record-player">by {record.playerName}</div>
											</div>
											<div className={`status-indicator ${getStatusColor(record.status)}`}>
												{record.status}
											</div>
										</div>
									))}
									...
									<div>
										...
										<div className="details-content">
											<div className="detail-item">
												<strong>Level:</strong> {selectedRecord.levelName}
											</div>
											<div className="detail-item">
												<strong>Player:</strong> {selectedRecord.playerName}
											</div>
											<div className="detail-item">
												<strong>Progress:</strong> {selectedRecord.progress}%
											</div>
											<div className="detail-item">
												<strong>Status:</strong> 
												<select
													value={selectedRecord.status}
													onChange={(e) => updateRecordStatus(selectedRecord.id, e.target.value)}
											...
											<div className="video-container">
												<iframe
													src={`https://www.youtube.com/embed/${extractYouTubeId(selectedRecord.video)}`}
													title="Record video"
													allowFullScreen
				...
				{activeTab === 'claims' && (
					<div className="tab-content">
						...
						<div className="claims-list">
								...
								<div key={claim.userId} className="claim-item">
									<div className="claim-info">
										...
										<div className="claim-player">
											<strong>Claims:</strong> {claim.playerName}
										</div>
									</div>
									<div className="claim-actions">
										<button
											className="action-button approve-button"
											onClick={() => handleClaim(claim.userId, claim.playerId, true)}
											title="Approve claim"
										>
											✓
										</button>
										<button
											...
											onClick={() => handleClaim(claim.userId, claim.playerId, false)}
											title="Reject claim"
										>
											✗
										</button>
							...
							{claims.length === 0 && (
								<div className="no-claims">No pending claims</div>
							)}
						</div>
					</div>
				)}
